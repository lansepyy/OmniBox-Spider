...
async function category(params) {
    try {
        ...
        if (typeId.includes('?')) {
            // 如果已经有查询参数
            pageUrl = `${typeId}&page=${currentPage}`;
        } else {
            // 如果没有查询参数，尝试不同的格式
            const testUrls = [
                `${typeId}?page=${currentPage}`,
                `${typeId}/${currentPage}.html`,
                `${typeId}/index_${currentPage}.html`,
                `${typeId}?p=${currentPage}`,
                `${typeId}/page/${currentPage}`,
            ];
            ...
            for (let testUrl of testUrls) {
                try {
                    logInfo("测试分页URL", { testUrl });
                    const testHtml = await request(testUrl);
                    // 检查是否包含视频内容
                    const $test = cheerio.load(testHtml);
                    const hasContent = $test("a[href*='/detail/']").length > 0;
                    if (hasContent) {
                        pageUrl = testUrl;
                        logInfo("找到有效分页URL", { pageUrl });
                        break;
                    }
                } catch(e) {
                    continue;
        ...
        $("a[href*='/detail/']").each((_, el) => {
            ...
            // 跳过广告链接
            if (href.includes('ad') || href.includes('advert') || href.includes('goto')) return;
            ...
            if (!title || title.length < 1 || title.length > 100) return;
            ...
            if (!vod_pic) {
                const bgStyle = a.attr("style") || "";
                const bgMatch = bgStyle.match(/background(?:-image)?:\s*url\(['"]?(.*?)['"]?\)/i);
                if (bgMatch) vod_pic = bgMatch[1];
            }
            vod_pic = toAbsUrl(vod_pic);
            
            // 提取备注
            let remarks = a.find(".remarks, .episode, .tag, .note").text().trim() || "";
        ...
        // 如果没有找到内容，可能是第一页没有内容，返回空列表
        if (list.length === 0 && currentPage === 1) {
            logInfo("第一页没有找到内容，可能是网站结构问题");
        }
        ...
        // 查找分页链接
        const paginationText = $(".page, .pagination, .pages, .page-numbers, .pager").text();
        const hasNextText = paginationText.includes("下一页") || 
                           paginationText.includes("下页") || 
                           paginationText.includes("next");
        
        // 查找下一页按钮
        const nextLink = $('a:contains("下一页"), a:contains("下页"), a:contains("next"), .next, .pagination-next, .page-next');
        if (nextLink.length && !nextLink.hasClass("disabled") && nextLink.attr("href")) {
            hasNextPage = true;
        }
        ...
        // 尝试从分页控件中获取总页数
        const lastPageLink = $('.page a:last-child, .pagination a:last-child, .pages a:last-child');
        if (lastPageLink.length) {
            const lastHref = lastPageLink.attr("href") || "";
            const pageMatch = lastHref.match(/[?&]page=(\d+)/) || 
                            lastHref.match(/\/(\d+)\.html/) ||
                            lastHref.match(/\/page\/(\d+)/);
            if (pageMatch) {
        ...
        // 从文本中提取总页数
        const pageText = $(".page, .pagination, .pages").text();
        const totalMatch = pageText.match(/共(\d+)页/) || pageText.match(/\/(\d+)/);
        if (totalMatch) {
            totalPages = Math.max(totalPages, parseInt(totalMatch[1]));
        }
